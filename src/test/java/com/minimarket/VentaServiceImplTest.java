 ...
 * - Campos faltantes en cÃ¡lculo total
 * - Cantidades negativas y cero
 * - Productos sin ID
 * - Pruebas parametrizadas
 * - PrecisiÃ³n monetaria
 */
...
class VentaServiceImplTest {
    ...
    @Test
    void registrarVentaRechazaAdministradorPorqueVentaEsOperacionDeCajero() {
        Venta venta = crearVentaConDetalles(1, 1);

        when(usuarioRepository.findById(1L)).thenReturn(Optional.of(crearUsuarioValido("ADMIN")));
    ...
    // ============= PRUEBAS DE VALIDACIÃ“N DE PRECIOS NULOS =============
    ...
    void calcularTotalConDetallesVacio_DebeRetornarCero() {
        ...
        // Assert
        assertEquals(0.0, total, "Venta con detalles vacÃ­o debe retornar 0");
    }

    // ============= PRUEBAS DE CANTIDADES INVÃLIDAS =============
    ...
    void calcularTotalConCantidadNegativa_DebeRechazar() {
        ...
        assertFalse(tieneStock, "Cantidad negativa no debe pasar validaciÃ³n de stock");
    }
    ...
    void calcularTotalConCantidadCero_DebeRechazar() {
        ...
        // Act & Assert
        boolean tieneStock = ventaService.tieneStockSuficiente(venta);
        assertFalse(tieneStock, "Cantidad cero no debe pasar validaciÃ³n de stock");
    }
    ...
    void calcularTotalConCantidadNula_DebeRechazar() {
        ...
        // Act & Assert
        boolean tieneStock = ventaService.tieneStockSuficiente(venta);
        assertFalse(tieneStock, "Cantidad nula no debe pasar validaciÃ³n");
    }
    ...
    void detalleVentaProductoNoEncontrado_DebeHandlear() {
        ...
        // Act & Assert
        boolean tieneStock = ventaService.tieneStockSuficiente(venta);
        assertFalse(tieneStock, "Producto no encontrado debe fallar en validaciÃ³n");
    }
    ...
    // ============= PRUEBAS PARAMETRIZADAS MÃšLTIPLES COMBINACIONES =============
    ...
    void calcularTotalMultiplesCombinaciones(Double precio, Integer cantidad) {
        ...
        assertEquals(esperado, total, 0.01, "CÃ¡lculo debe ser preciso para " + precio + " x " + cantidad);
    }

    // ============= PRUEBAS DE PRECISIÃ“N MONETARIA =============

    @Test
    void calcularTotalSinErroresRedondeo_DebeMantenerlaPrecision() {
        // Arrange - SimulaciÃ³n de cÃ¡lculo con mÃºltiples detalles
        Venta venta = new Venta();
        Usuario usuario = crearUsuarioValido("EMPLEADO");
        venta.setUsuario(usuario);
        venta.setFecha(new Date());
        ...
        assertEquals(esperado, total, 0.01, "CÃ¡lculo monetario debe mantener precisiÃ³n");
    }
